...
if(mb_strlen($email) < 10 || mb_strlen($email) > 50){
    echo "Email должен быть не менее 10 или не более 50 симболов";
    exit();
} elseif (mb_strlen($login) < 8 || mb_strlen($login) > 20) {
    echo "Логин должна быть не менее 8 или не более 20 симболов";
    exit();
} elseif (mb_strlen($name) < 3 || mb_strlen($name) > 25) {
    echo "Имя должен быть не менее 8 или не более 20 симболов";
    exit();
} elseif (mb_strlen($pass) < 8 || mb_strlen($pass) > 20) {
    echo "Пароль должен быть не менее 8 или не более 20 симболов";
    exit();
}
$pass = md5($pass);
$pass2 = md5($pass2);
