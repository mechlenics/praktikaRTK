...
if(isset($_GET['true'])) {
    ...
    $result = $mysql->query("SELECT * FROM `trip` WHERE `Where from` = '$search' AND `__Where__` = '$search1' AND `Day`='$date'");
}elseif(isset($_GET['button'])) {
    $result = $mysql->query("SELECT * FROM `trip` WHERE `Where from` = '$search' AND `__Where__` = '$search1'");
  		...
  		<div class="row">
		    <div class="col-6">
                <form action="Rez.php" method="get">
                    ...
                    <input type="submit" name="false" value="Показать все поездки" class="btn btn-success center" style="margin-left: -9%; margin-top: -200px">
                </form>
            </div>
		    <div class="col-6"></div>
  		</div>
		    	...
		    	<table class="table table-hover table-info">
  					<thead>
					    <tr>
					     	...
					     	<th scope="col">Время в дороге</th>
                        ...
                        <tr>
                            ...
                            <td>
					      		<form action="Ofor.php" method="get">
                                    ...
                                    <input type="hidden" name="Time_in_road" value="<?=$user['Time in road']?>">
